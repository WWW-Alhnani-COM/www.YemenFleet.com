            ...
            <form action="{{ route('admin.inventory.update', $inventory->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="product_name" id="product_name" value="{{ old('product_name', $inventory->product_name) }}"
                    ...
                    <div>
                        ...
                        <select name="company_id" id="company_id"
                            ...
                            @foreach($companies as $company)
                                <option value="{{ $company->id }}" {{ (old('company_id', $inventory->company_id) == $company->id ? 'selected' : '') }}>{{ $company->company_name }}</option>
                            @endforeach
                        </select>
                        @error('company_id')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="price" id="price" value="{{ old('price', $inventory->price) }}"
                               class="w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200"
