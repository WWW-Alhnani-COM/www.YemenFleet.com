...
@section('content')
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <a href="{{ route('admin.notifications.create') }}"
               class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 transition duration-200 dark:bg-indigo-700 dark:hover:bg-indigo-600 flex items-center justify-center">
                <i class="fas fa-plus mr-2"></i> إرسال إشعار جديد
            </a>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow mb-6">
        <form action="{{ route('admin.notifications.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <!-- فلترة حسب نوع المستقبل -->
            <div>
                <label for="receiver_type" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">نوع المستقبل</label>
                <select name="receiver_type" id="receiver_type"
                    ...
                    <option value="App\Models\Company" {{ request('receiver_type') == 'App\Models\Company' ? 'selected' : '' }}>شركات</option>
                    <option value="App\Models\User" {{ request('receiver_type') == 'App\Models\User' ? 'selected' : '' }}>عملاء</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="is_read" id="is_read"
                       class="w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200">
                    <option value="">الكل</option>
                    <option value="true" {{ request('is_read') == 'true' ? 'selected' : '' }}>مقروء</option>
                    <option value="false" {{ request('is_read') == 'false' ? 'selected' : '' }}>غير مقروء</option>
                </select>
            </div>

            <!-- فلترة حسب نوع الإشعار -->
            <div>
                <label for="is_group" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">نوع الإشعار</label>
                <select name="is_group" id="is_group"
                       class="w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200">
                    <option value="">الكل</option>
                    <option value="true" {{ request('is_group') == 'true' ? 'selected' : '' }}>جماعي</option>
                    <option value="false" {{ request('is_group') == 'false' ? 'selected' : '' }}>فردي</option>
                </select>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">المرسل</th>
                        <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">المستقبل</th>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="mr-4">
                                    <div class="text-sm font-medium text-gray-900 dark:text-gray-200">{{ $notification->sender->name ?? 'غير معروف' }}</div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400">({{ class_basename($notification->sender_type) }})</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
    ...
    @endif
                            ...
                            <div class="text-sm text-gray-500 dark:text-gray-400">({{ class_basename($notification->notifiable_type) }})</div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-900 dark:text-gray-200 line-clamp-2">{{ Str::limit($notification->message, 70) }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
        ...
        <i class="fas fa-edit"></i>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                            @if(request()->anyFilled(['receiver_type', 'is_read', 'is_group']))
