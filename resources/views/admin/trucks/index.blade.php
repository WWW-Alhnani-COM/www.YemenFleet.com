...
@section('content')
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <a href="{{ route('admin.trucks.create') }}"
               class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 transition duration-200 dark:bg-indigo-700 dark:hover:bg-indigo-600 flex items-center justify-center">
                <i class="fas fa-plus mr-2"></i> إضافة شاحنة جديدة
            </a>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow mb-6">
        <form action="{{ route('admin.trucks.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <!-- فلترة حسب اسم الشاحنة -->
            <div>
                <label for="truck_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">اسم الشاحنة</label>
                <input type="text" name="truck_name" id="truck_name" value="{{ request('truck_name') }}"
                       ...
                       placeholder="ابحث باسم الشاحنة">
            </div>

            <!-- فلترة حسب رقم اللوحة -->
            <div>
                <label for="plate_number" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">رقم اللوحة</label>
                <input type="text" name="plate_number" id="plate_number" value="{{ request('plate_number') }}"
                       class="w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200"
                       placeholder="ابحث برقم اللوحة">
            </div>

            <!-- فلترة حسب حالة الشاحنة -->
            <div>
                ...
                <select name="vehicle_status" id="vehicle_status"
                    ...
                    <option value="نشطة" {{ request('vehicle_status') == 'نشطة' ? 'selected' : '' }}>نشطة</option>
                    <option value="متوقفة" {{ request('vehicle_status') == 'متوقفة' ? 'selected' : '' }}>متوقفة</option>
                    <option value="تحت الصيانة" {{ request('vehicle_status') == 'تحت الصيانة' ? 'selected' : '' }}>تحت الصيانة</option>
                </select>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">اسم الشاحنة</th>
                        <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">رقم اللوحة</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="mr-4">
                                    <div class="text-sm font-medium text-gray-900 dark:text-gray-200">{{ $truck->truck_name }}</div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400">الشاصي: {{ $truck->chassis_number }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $truck->plate_number }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                            {{ $truck->company->company_name }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center space-x-2 space-x-reverse">
                                ...
                                <form action="{{ route('admin.trucks.destroy', $truck->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                            @if(request()->anyFilled(['truck_name', 'plate_number', 'vehicle_status', 'company_id']))
