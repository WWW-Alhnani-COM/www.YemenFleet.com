...
<div class="container mx-auto p-6 max-w-4xl">
    <h1 class="text-3xl font-bold mb-6 text-gray-900 dark:text-white">تعديل الوجهة</h1>
    ...
    <form action="{{ route('admin.destinations.update', $destination->id) }}" method="POST" 
        class="bg-gray-100 dark:bg-gray-800 p-6 rounded shadow" >
        @csrf
        @method('PUT')
            ...
            <select name="task_id" id="task_id" required
                ...
                @foreach($tasks as $task)
                    <option 
                        ...
                        @else
                            @selected($destination->task_id == $task->id)
                        @endif
                    >
        ...
        <div class="mb-4">
            ...
            <input type="text" name="start_point" id="start_point"
                value="{{ old('start_point', $destination->start_point) }}" required
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="number" name="start_latitude" id="start_latitude"
                    value="{{ old('start_latitude', $destination->start_latitude) }}" step="0.000001" required
            ...
            <div>
                ...
                <input type="number" name="start_longitude" id="start_longitude"
                    value="{{ old('start_longitude', $destination->start_longitude) }}" step="0.000001" required
        ...
        <div class="mb-4">
            ...
            <input type="text" name="end_point" id="end_point" value="{{ old('end_point', $destination->end_point) }}" required
        ...
        <div class="mb-4">
            ...
            <textarea name="end_address" id="end_address" rows="3" required
                class="w-full rounded border border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="العنوان التفصيلي لنقطة النهاية">{{ old('end_address', $destination->end_address) }}</textarea>
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="datetime-local" name="date" id="date"
                value="{{ old('date', $destination->date ? $destination->date->format('Y-m-d\TH:i') : '') }}" required
                class="w-full rounded border border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
