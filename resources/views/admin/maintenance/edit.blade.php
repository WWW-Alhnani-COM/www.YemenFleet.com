            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="truck_id" id="truck_id" 
                        ...
                        <option value="{{ $truck->id }}" @if(old('truck_id', $maintenance->truck_id)==$truck->id) selected @endif>
                ...
                <div>
                    ...
                    <select name="type" id="type" 
                        ...
                        <option value="preventive" @if(old('type', $maintenance->type)=='preventive') selected @endif>صيانة وقائية</option>
                        <option value="corrective" @if(old('type', $maintenance->type)=='corrective') selected @endif>صيانة تصحيحية</option>
                        <option value="emergency" @if(old('type', $maintenance->type)=='emergency') selected @endif>صيانة طارئة</option>
                    </select>
                    @error('type')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="datetime-local" name="date" id="date" 
                           class="w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200" 
                           value="{{ old('date', $maintenance->date->format('Y-m-d\TH:i')) }}" required>
                    @error('date')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200">{{ old('description', $maintenance->description) }}</textarea>
                    @error('description')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
