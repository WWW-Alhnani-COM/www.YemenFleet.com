                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="method" id="method" required
                            ...
                            <option value="credit_card" {{ old('method', $payment->method) == 'credit_card' ? 'selected' : '' }}>بطاقة الائتمان</option>
                            <option value="نقدي" {{ old('method', $payment->method) == 'نقدي' ? 'selected' : '' }}>نقدي</option>
                            <option value="bank_transfer" {{ old('method', $payment->method) == 'bank_transfer' ? 'selected' : '' }}>تحويل بنكي</option>
                            <option value="آجل" {{ old('method', $payment->method) == 'آجل' ? 'selected' : '' }}>آجل</option>
                        </select>
                        @error('method')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="date" id="date" 
                               value="{{ old('date', $payment->date ? $payment->date->format('Y-m-d\TH:i') : '') }}" required
                               class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200">
                        @error('date')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <!-- <div>
                        ...
                        <input type="number" name="subscription_id" id="subscription_id" 
                               value="{{ old('subscription_id', $payment->subscription_id) }}"
                               class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200"
                               placeholder="اختياري">
                    ...
                    <!-- <div>
                        ...
                        <input type="number" name="company_order_id" id="company_order_id" 
                               value="{{ old('company_order_id', $payment->company_order_id) }}"
                               class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200"
                               placeholder="اختياري">
